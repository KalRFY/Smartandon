<template>
  <CContainer fluid>
    ...
    <CTable hover bordered v-else>
      ...
      <CTableBody>
        <CTableRow
          v-for="(item, i) in filteredKyData"
          :key="item.machine_id || i"
        >
    ...
    <CModal :visible="dialog" @close="clearSubmit" size="lg">
      ...
      <CModalBody>
        ...
        <CButton
          ...
          >Add +</CButton
        >
        <CButton color="danger" size="sm" v-else @click="isCardVisible = false"
          >Close X</CButton
        >

        <CCard v-if="isCardVisible" class="mt-3 p-3">
          <CFormInput
            v-model="kyObj.details"
            label="Description KY"
            placeholder="Enter KY Description"
          />
          <CFormLabel class="mt-3">Stop 6 Category</CFormLabel>
          <CFormSelect v-model="kyObj.stop6_category">
            <option
              v-for="cat in stop6Categories"
              :key="cat.id"
              :value="cat.id"
            >
              {{ cat.label }}
          ...
          <CButton
            ...
            color="primary"
            block
            :disabled="!isKyFullFill"
            @click="submitKy"
            >Submit
          </CButton>
        </CCard>

        <CBadge color="primary" v-if="isEditLabel" class="mt-3"
          >Edit Mode</CBadge
        >
        ...
        <CTable bordered responsive class="mt-3">
          ...
          <CTableBody>
            <CTableRow v-for="(ky, i) in detailsKyData" :key="ky.id">
              <CTableDataCell>{{ i + 1 }}</CTableDataCell>
              <CTableDataCell v-if="!ky.is_edit">{{
                ky.details
              }}</CTableDataCell>
...
<script>
import { getKYData, addKY, editKY, deleteKY } from '@/apis/smartandon'
...
export default {
  ...
  methods: {
    ...
    async getKy() {
      try {
        this.isLoading = true
        const flineParam = this.filter.fline === '-1' ? '' : this.filter.fline
        const response = await getKYData(flineParam)
        this.kyData = response.data?.data?.[0] || []

        const apiFilterLine = response.data?.data?.[1]
        if (apiFilterLine) {
    ...
    applyFilter() {
      ...
      if (this.filter.fline !== '-1' && this.filter.fline !== 'ALL') {
        const selectedLine = this.lineOptions
          .find((opt) => opt.value == this.filter.fline)
          ?.text?.toLowerCase()
      ...
      if (this.filter.fid !== '-1' && this.filter.fid !== 'ALL') {
        const selectedMachine = this.machineOptions
          .find((opt) => opt.value == this.filter.fid)
          ?.text?.toLowerCase()
