...
// Emit definition
const emit = defineEmits(['refresh', 'view-item', 'edit-item', 'delete-item'])
...
const destroyChart = () => {
  if (chartInstance.value) {
    try {
      if (chartInstance.value.options && chartInstance.value.options.animation) {
  ...
  return {
    ...
    options: {
      ...
      plugins: {
        title: {
          display: true,
          text: `${props.title} - ${props.viewMode === 'machines' ? 'Machine' : 'Problem'} ${props.metricMode === 'duration' ? 'Duration' : 'Frequency'} Chart`,
      ...
      scales: {
        x: { 
          title: { 
            display: true, 
            text: props.viewMode === 'machines' ? 'Machines' : 'Problems' 
          }
        },
        y: {
          display: true,
