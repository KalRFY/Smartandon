<template>
  ...
  >
    <div class="d-flex flex-column w-100 h-100 align-items-center">
      ...
      <div
        class="bg-white rounded shadow w-layout"
      ...
      >
        ...
        >
          <span
            v-if="!isLoading && (!resultSearch || resultSearch.length === 0)"
            style="opacity: 0.5"
            >Search Machine / Line / Item Check / Schedule / History</span
          >
          <div v-else>
        ...
        <template v-else>
          <div class="py-2 px-3 h-100">
            <template v-for="(parent, i) in resultSearch" :key="i">
              <template
                v-if="Array.isArray(parent?.child) && parent?.child?.length"
              >
                <div class="ms-2 mb-1 text-start">
                  <h6 class="fw-bold">{{ parent.category }}</h6>
                </div>
                <div
                  v-for="(item, i) in parent.child"
                  :key="i"
                  class="row m-2 item p-2"
                >
                  <div
                    class="col-12 col-md-12 w-100"
                  ...
                  >
                    <div class="d-flex gap-2">
                      ...
                      <div class="d-flex flex-column text-start flex-grow-1">
                        ...
                        <p v-if="item.itemName" class="mb-0">
                          {{ item.itemName }}
                        </p>
                      </div>
              ...
              <template v-else>
                ...
                >
                  <span class="text-center"> Pencarian tidak ditemukan </span>
                </div>
              </template>
            </template>
          </div>
    ...
    async onSearch() {
      ...
      try {
        ...
        if (result.status === 200) {
          ...
          this.resultSearch = [...this.resultSearch, features && features]
        }
      } catch (e) {
    ...
    mappedMenu() {
      ...
      if (this.search) {
        menu.child = menu.child.filter((item) =>
          item.featureName.toLowerCase().includes(this.search.toLowerCase()),
        )
      }

      if (menu.child.length) {
        this.resultSearch = [menu]
      } else {
        this.resultSearch = []
      }
    },
