...
module.exports = (sequelize, Sequelize) => {
  class GaugeService {
     ...
     */
    static async createGauge(Gauge, userId) {
      console.log(Gauge);
     ...
     * @returns {Promise<Gauge>}
     */
    static async updateGauge(Gauge, userId) {
      return new Promise(async (resolve, reject) => {
        await sequelize
          .query(
    ...
    static async paginateGauge(filter, options, userId) {
      return new Promise(async (resolve, reject) => {
        ...
        await sequelize
          ...
          .then((result) => {
            ...
            } else if (result[0][0]?.FNAME === 'ROW_COUNT') {
              const rowCount = result[0].splice(0, 1);
              resolve({ count: rowCount[0].FVAL, rows: result[0] });
            } else {
              resolve(result);
            }
          })
     ...
     */
    static async queryGauges(filter, options, userId) {
      return await this.paginateGauge(filter, options, userId);
    }
     ...
     * @returns {Promise<Gauge>}
     */
    static async deleteGauge(Gauge, userId) {
      return await sequelize.query(
        'qdc_gauge_delete_data' + ' @gaugeId = :gauge_id,' + " @changedBy = '" + userId + "'",
        { replacements: Gauge },
        Sequelize.QueryTypes.UPDATE
      );
    }
     ...
     * @returns {Promise<Gauge>}
     */
    static async getGauge(Gauge, userId) {
      return new Promise(async (resolve, reject) => {
        await sequelize
          .query(
            'qdc_gauge_get_data' + ' @gaugeId = :gauge_id,' + " @searchBy = '" + userId + "'",
            { replacements: Gauge },
            Sequelize.QueryTypes.SELECT
          )
          .then((result) => resolve(result))
